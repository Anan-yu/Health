    ...
    def evaluate(self, request: AssessmentRequest) -> list[ModelResult]:
        ...
        return [
            ModelResult(
                ...
                riskLevel="ATTENTION" if glucose_attention else "LOW",
                evidence=(
                    ["示例：空腹血糖高于演示关注阈值"]
                    if glucose_attention
                    else ["示例：现有糖代谢指标未触发演示关注规则"]
                ),
                missingIndicators=([] if "fasting_insulin" in values else ["fasting_insulin"]),
                recommendations=["建议由专业人员结合完整健康信息进行审核"],
            ),
            ModelResult(
                modelCode="INFLAMMATION",
                modelName="慢性炎症评估",
                score=65 if inflammation_attention else 88,
                riskLevel="ATTENTION" if inflammation_attention else "LOW",
                evidence=(
                    ["示例：C反应蛋白高于演示关注阈值"]
