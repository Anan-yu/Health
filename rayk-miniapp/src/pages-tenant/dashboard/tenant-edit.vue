<template>
  <view class="page admin-page">
    <view class="page-heading">
      ...
      <view class="subtitle">平台统一维护医院资料，并预录入医生手机号供微信授权登录。</view>
    </view>
    <PageState :loading="loading" :error="error" :empty="!form">
      ...
      <view class="staff-heading"
        ><view>医生管理</view><text>{{ doctors.length }} 位</text></view
      >
      <view class="card staff-card">
        <view class="staff-tip"
          >预录入医生手机号后，医生可直接通过微信授权手机号登录，无需绑定码。</view
        >
        <input
        ...
        <input
          ...
          placeholder="医生手机号"
        />
        <button class="secondary staff-save" :loading="doctorSaving" @click="saveDoctor">
          预录入医生
        </button>
      </view>
      <view v-for="doctor in doctors" :key="doctor.id" class="card doctor-row">
        ...
        <view class="doctor-info"
          ...
          ><view class="doctor-phone">{{ doctor.phoneMasked }}</view></view
        >
        ...
        <view v-if="editingDoctorId === doctor.id" class="doctor-edit">
          <input
            v-model="editDoctorForm.displayName"
            class="input"
            maxlength="50"
            placeholder="医生姓名"
          />
          <input
            v-model="editDoctorForm.phone"
            class="input staff-input"
            type="number"
            maxlength="11"
            placeholder="如需更换手机号，请输入新号码"
          />
          <view class="edit-tip">不填写新手机号时，将保留当前手机号。</view>
          <view class="edit-actions"
            ><button class="secondary edit-cancel" @click="cancelEditDoctor">取消</button
            ><button
              class="primary edit-save"
...
async function load() {
  ...
  try {
    const [tenant, staff] = await Promise.all([
      getPlatformTenant(tenantId.value),
      getPlatformDoctors(tenantId.value),
    ])
    form.value = {
    ...
    error.value = cause instanceof Error ? cause.message : '医院信息加载失败'
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  const tenantName = form.value.tenantName.trim(),
    servicePlan = form.value.servicePlan.trim()
  if (!tenantName || !servicePlan) {
    formError.value = '请填写医院名称和服务套餐'
    return
  }
  saving.value = true
  formError.value = ''
...
async function saveDoctor() {
  const displayName = doctorForm.displayName.trim(),
    phone = doctorForm.phone.trim()
  if (!displayName || !/^1[3-9]\d{9}$/.test(phone)) {
    uni.showToast({ title: '请输入医生姓名和有效手机号', icon: 'none' })
    return
  }
  doctorSaving.value = true
  try {
    await createPlatformDoctor(tenantId.value, { displayName, phone })
    doctorForm.displayName = ''
    doctorForm.phone = ''
    await load()
    uni.showToast({ title: '医生已预录入', icon: 'success' })
  } catch (cause) {
    uni.showToast({ title: cause instanceof Error ? cause.message : '预录入失败', icon: 'none' })
  } finally {
...
async function updateDoctor(doctorId: string) {
  const displayName = editDoctorForm.displayName.trim(),
  ...
  try {
    ...
    uni.showToast({
      title: cause instanceof Error ? cause.message : '修改失败，请稍后重试',
      icon: 'none',
    })
  } finally {
    doctorSaving.value = false
...
async function removeDoctor(doctor: TenantStaff) {
  ...
  try {
    ...
    uni.showToast({
      title: cause instanceof Error ? cause.message : '删除失败，请稍后重试',
      icon: 'none',
    })
  } finally {
    doctorSaving.value = false
