...
const metricDefinitions: MetricDefinition[] = [
  ...
  },
  {
    ...
    explanation:
      '心率变异性反映身体压力和心脏调节能力。若持续偏低，可能与心律失常、冠心病、心力衰竭、糖尿病自主神经损伤等疾病有关。单次异常不能诊断疾病，若反复偏低或伴有心慌、胸闷、头晕，应及时就医检查。',
    ranges: [
    ...
    assess: (result) => {
      ...
      if (value < 30) {
        return attentionAssessment(
          '偏低',
          '本次心率变异性偏低',
          '本次心率变异性偏低。短期偏低常见于睡眠不足、过度疲劳、精神紧张或恢复不充分；如果持续低于个人平时水平，并伴有心悸、胸闷、头晕、乏力，需排查自主神经调节异常，以及睡眠障碍、感染恢复期或心血管/代谢方面的风险。',
          '连续3—7天在相近时间、安静状态下复测，并记录睡眠、压力、静息心率和症状；若持续偏低或伴随不适，建议携带趋势记录咨询医生。',
          '持续偏低需要关注',
          '单次偏低不等于患病，但持续低于个人基线可能提示身体处于压力、疲劳或恢复不足状态，也可能与睡眠障碍、感染恢复期及心血管/代谢问题有关；医生可能结合心电图等检查进一步排查心律失常或其他心血管风险。',
          '如出现持续心悸、胸闷、晕厥、明显头晕或呼吸困难，请及时就医，不要仅依赖面部检测结果。',
        )
      }
      if (value > 120) {
        return normalAssessment(
          '偏高',
          '本次心率变异性高于常见范围',
          '心率变异性个体差异较大，本次值高于常见参考范围，可能受测量时呼吸、姿势、放松状态或算法差异影响，不能据此判断疾病；需要结合个人长期基线和其他症状复核。',
      ...
      return normalAssessment(
        '平稳',
        '本次心率变异性较平稳',
        '本次结果未见明显偏低，但单次测量不能代表长期状态，也不能排除潜在问题；更重要的是观察个人基线和连续趋势。',
        '保持规律睡眠、适度运动和情绪放松，并在相近时间持续复测；如趋势持续下降或出现不适，建议进一步咨询医生。',
      )
    },
  },
  {
