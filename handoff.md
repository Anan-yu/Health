...
## 0.5 2026-07-24 受控健康随访闭环

- 用户反馈由整段自由文本改为逐项对照反馈：每个健康行动必须选择“已完成、部分完成、未完成”，未完全执行时可补充原因，另保留整期补充说明。
- 服务端依据逐项结果计算真实完成度：已完成 100 分、部分完成 50 分、未完成 0 分，并保存结构化反馈、完成率、决策结果和决策原因。
- 健康随访采用受控循环，默认最多 4 期：完成度低于 60% 时下一期聚焦未完成行动；达到 60% 后继续下一期；连续两期达到 80% 或已到第 4 期时自动结束，不再无限生成任务。
- 新健康报告发布时，自动结束旧的待完成计划并从第 1 期重新建立，避免新旧计划并行冲突。
- 到期前一天、到期当天及逾期阶段会在消息中心形成提醒状态；逾期超过 14 天仍未反馈时，本轮随访自动暂停，不再继续派发下一期。
- 平台管理员的任务详情页可查看本期完成率、当前期数、继续/调整/结束决策、决策原因以及用户每一项行动的实际反馈。
- 数据库迁移升级至 V27；Java 52 项测试、前端 TypeScript 与 ESLint 均通过，H5、开发微信包和生产局域网微信包已同步重建。
