...
- H5 查看原检验报告和下载 PDF 健康报告改用带鉴权的 Java 文件接口生成临时浏览地址，不再依赖浏览器直连 MinIO 的 `127.0.0.1:9000` 预签名地址；微信端原有下载路径保持不变。
...
- 每个疾病推断参考均要求给出“专科确认与分层—循证治疗类别—疗效复核/随访”的具体路径，不能只写“由专科决定”。知识库版本 `2.1.0` 已加入幽门螺杆菌指南证据；可明确“含铋四联根除治疗路径”等治疗类别，但药物组合、剂量和适用性仍必须由医生确认，禁止生成自行购药或自行调整用药的内容。
